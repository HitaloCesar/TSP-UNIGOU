...
class SolutionViewer:
    ...
    def handleEvolution(self):
        ...
        assert isinstance(self.ga, GeneticAlgorithm)
        
        #Adding SHOW_LAST entries to the list
        indexes = self.ga.getNStrongestIdx(SHOW_LAST)
        for idx in indexes:
            newEntry = ( 
                self.ga.population[idx],
                self.ga.fitness[idx],
                self.ga.currGeneration,
            )
            self.bestTuples.append(newEntry)
        
        #Sorting, keeping first entries and requesting a display update
        self.bestTuples.sort(key=itemgetter(1), reverse=True) #order by fitness
